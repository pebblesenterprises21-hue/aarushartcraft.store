...
return    <article class="product-card" data-testid="product-card-${p.id}">   <div class="product-img-wrap" onclick="window.location.href='product.html?id=${p.id}'">   <span class="stock-tag ${stockClass}">${p.stock}</span>   <button class="wish-btn ${wished ? "active" : ""}" data-wish-id="${p.id}"   onclick="event.stopPropagation(); toggleWish(${p.id})"   data-testid="wish-toggle-${p.id}" aria-label="Wishlist">   <i class="fa-${wished ? "solid" : "regular"} fa-heart"></i>   </button>   <img src="${p.image}" alt="${p.name}" loading="lazy">   </div>   <div class="product-body">   <div class="product-cat">${cat ? cat.name : p.category}</div>   <a href="product.html?id=${p.id}"><h3 class="product-name">${p.name}</h3></a>   <p class="product-desc">${p.description}</p>   <div class="product-foot">   <div class="product-price">${fmt(p.price)} <small>incl. taxes</small></div>   <button class="add-btn" onclick="addToCart(${p.id})"   data-testid="add-cart-${p.id}">   <i class="fa-solid fa-bag-shopping"></i> Add   </button>   </div>   </div>   </article>;
}
...
el.innerHTML = CATEGORIES.map(c =>   <a class="cat-card" href="products.html?cat=${c.id}#products-grid" data-testid="cat-${c.id}">   <div class="cat-icon">   <i class="fa-solid ${c.icon}"></i>   </div>   <h3>${c.name}</h3>   </a>  ).join("");
}
...
if (items.length === 0) {
el.innerHTML = <div class="list-empty" style="grid-column:1/-1">   <i class="fa-solid fa-magnifying-glass"></i>   <h3>No matches found</h3>   <p>Try a different category or search term.</p>   </div>;
return;
}
el.innerHTML = items.map(productCardHTML).join("");
}
...
if (!p) {
wrap.innerHTML = <div class="list-empty">   <i class="fa-solid fa-circle-question"></i>   <h3>Product not found</h3>   <p>The product you're looking for doesn't exist.</p>   <a href="products.html" class="btn btn-primary">Browse all products</a>   </div>;
return;
}
const cat = CATEGORIES.find(c => c.id === p.category);
const wished = getWish().includes(p.id);
...
const cart = getCart();
if (cart.length === 0) {
wrap.innerHTML = <div class="list-empty" data-testid="cart-empty">   <i class="fa-solid fa-bag-shopping"></i>   <h3>Your cart is empty</h3>   <p>Looks like you haven't picked anything yet.</p>   <a href="products.html" class="btn btn-primary">Start shopping</a>   </div>;
return;
}
...
const ids = getWish();
if (ids.length === 0) {
wrap.innerHTML = <div class="list-empty" data-testid="wish-empty">   <i class="fa-regular fa-heart"></i>   <h3>No favourites yet</h3>   <p>Tap the heart on any product to save it for later.</p>   <a href="products.html" class="btn btn-primary">Browse products</a>   </div>;
return;
}
...
modal.innerHTML =    <div class="modal" data-testid="upi-modal">   <button class="modal-close" onclick="closeUPI()" data-testid="upi-close">   <i class="fa-solid fa-xmark"></i>   </button>   <h3>Pay via UPI</h3>   <p>Scan the QR with any UPI app or use the ID below</p>   <div class="qr-box"><img src="${qrSrc}" alt="UPI QR Code"></div>   <div class="upi-id-box">   <span id="upi-id-text">${SHOP_CONFIG.upiId}</span>   <button class="copy-btn" onclick="copyUPI()" data-testid="upi-copy">Copy</button>   </div>   <a class="btn btn-primary" href="${upiUrl}" style="width:100%;justify-content:center" data-testid="upi-open-app">   Open UPI App · Pay ${fmt(amount)}   </a>   <p style="font-size:0.8rem;color:var(--ink-soft);margin-top:18px;margin-bottom:0">   After paying, send the screenshot to <strong>${SHOP_CONFIG.phone}</strong> on WhatsApp to confirm.   </p>   </div>;
...
if (foot) {
foot.innerHTML =   <footer>   <div class="container">   <div class="foot-grid">   <div>   <div class="foot-brand">Aarush <span>Art & Craft</span></div>   <p>${SHOP_CONFIG.tagline}<br>Crafted, painted and packed by hand.</p>   <div class="foot-socials">   <a href="${SHOP_CONFIG.instagram}" aria-label="Instagram"><i class="fa-brands fa-instagram"></i></a>   <a href="${SHOP_CONFIG.facebook}" aria-label="Facebook"><i class="fa-brands fa-facebook-f"></i></a>   <a href="https://wa.me/${SHOP_CONFIG.whatsapp}" aria-label="WhatsApp"><i class="fa-brands fa-whatsapp"></i></a>   </div>   </div>   <div>   <h4>Shop</h4>   <a href="products.html">All products</a>   ${CATEGORIES.slice(0, 4).map(c =><a href="products.html?cat=${c.id}">${c.name}</a>).join("")}   </div>   <div>   <h4>Help</h4>   <a href="contact.html">Contact</a>   <a href="cart.html">Your cart</a>   <a href="wishlist.html">Wishlist</a>   </div>   <div>   <h4>Reach us</h4>   <p>${SHOP_CONFIG.phone}<br>${SHOP_CONFIG.email}<br>${SHOP_CONFIG.address}</p>   </div>   </div>   <div class="foot-bottom">© ${new Date().getFullYear()} ${SHOP_CONFIG.brand}. All rights reserved.</div>   </div>   </footer>;
}
