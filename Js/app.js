...
function renderCategories(container) {
  ...
  el.innerHTML = CATEGORIES.map(c => `
    <a class="cat-card" href="products.html?cat=${c.id}" data-testid="cat-${c.id}">
...
function initProductsPage() {
  ...
  if (q) { currentQuery = q; $("#search-input").value = q; }

  // Chips
  const chipBar = $("#filter-bar");
  chipBar.innerHTML = `
    <button class="chip ${currentCat === "all" ? "active" : ""}" data-cat="all" data-testid="chip-all">All</button>
    ${CATEGORIES.map(c => `<button class="chip ${currentCat === c.id ? "active" : ""}" data-cat="${c.id}" data-testid="chip-${c.id}">${c.name}</button>`).join("")}
  `;
  chipBar.addEventListener("click", e => {
    const btn = e.target.closest(".chip");
    if (!btn) return;
    currentCat = btn.dataset.cat;
    $$(".chip", chipBar).forEach(c => c.classList.toggle("active", c === btn));
    renderProducts();
  });
...
function waLinkSingle(p) {
  const msg = `Hi ${SHOP_CONFIG.brand}! I'd like to order:%0A%0A• ${p.name}%0A• Price: ${fmt(p.price)}%0A%0APlease confirm availability and shipping. Thank you!`;
  return `https://wa.me/${SHOP_CONFIG.whatsapp}?text=${msg}`;
}
function waLinkCart(items, total) {
  const lines = items.map(p => `• ${p.name} × ${p.qty} — ${fmt(p.price * p.qty)}`).join("%0A");
  const msg = `Hi ${SHOP_CONFIG.brand}! I'd like to place this order:%0A%0A${lines}%0A%0ATotal: ${fmt(total)}%0A%0APlease confirm and share shipping details. Thank you!`;
  return `https://wa.me/${SHOP_CONFIG.whatsapp}?text=${msg}`;
}
